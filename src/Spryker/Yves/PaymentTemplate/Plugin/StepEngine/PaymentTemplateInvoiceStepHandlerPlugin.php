 ...
 */
class PaymentTemplateInvoiceStepHandlerPlugin extends AbstractPlugin implements StepHandlerPluginInterface
{
    /**
    ...
    public function addToDataClass(Request $request, AbstractTransfer $quoteTransfer): QuoteTransfer
    {
        ...
        $paymentTransfer->setPaymentMethod(PaymentTemplateConfig::PAYMENT_METHOD_INVOICE);
        $paymentTransfer->setPaymentSelection(PaymentTemplateConfig::PAYMENT_METHOD_INVOICE);
        $paymentTransfer->setPaymentTemplate(
            (new PaymentTemplateTransfer()),
            //->setAmount(1000)
        );
        $paymentTransfer->setPaymentTemplateInvoice(
            (new PaymentTemplateInvoiceTransfer()),
