...
/**
 * Spryker Payment Template Renamer
 *
...
class PaymentTemplateRenamer
{
    private const VALIDATION_PATTERN = '/^[a-z]+(-[a-z]+)*$/';

    private const MIN_NAME_LENGTH = 2;

    private const TEMPLATE_PASCAL = 'PaymentTemplate';

    private const TEMPLATE_CAMEL = 'paymentTemplate';

    private const TEMPLATE_KEBAB = 'payment-template';

    private const TEMPLATE_SNAKE = 'payment_template';
    ...
    private function parseArguments(array $argv): void
    {
        ...
        if ($projectPath !== null) {
            ...
            if (!$this->validateProjectPath()) {
                throw new Exception('Invalid project path. Must be a valid Spryker project with src/ directory.');
            }
    ...
    private function validateSource(): bool
    {
        ...
        foreach ($requiredPaths as $path) {
            ...
            if (!file_exists($fullPath)) {
                $this->error("Invalid payment-template structure. Missing: {$path}");
    ...
    private function validateProjectPath(): bool
    {
        ...
        // Check if it's a valid Spryker project
        $srcPath = $this->projectPath . DIRECTORY_SEPARATOR . 'src';
        if (!is_dir($srcPath)) {
            $this->error('Not a valid Spryker project. Missing src/ directory.');
    ...
    private function confirm(): bool
    {
        ...
        if ($this->projectMode) {
            ...
            $this->output("    - Rename files containing 'PaymentTemplate'\n");
    ...
    {
        $finder = new Finder();
        $finder->files()->in($this->sourcePath)->name('*PaymentTemplate*');
    ...
    /**
     * Rename files and directories containing 'PaymentTemplate'
     */
    ...
    private function renameFilesInPath(string $path): void
    {
        ...
        $finder = new Finder();
        $finder->files()->in($path)->name(['*PaymentTemplate*', '*paymentTemplate*', '*payment_template*', '*payment-template*']);
        ...
        $finder = new Finder();
        $finder->directories()->in($path)->name(['*PaymentTemplate*', '*paymentTemplate*', '*payment_template*', '*payment-template*']);
    ...
    private function updateFileContent(string $filePath): void
    {
        ...
        // 1. PascalCase - replaces PaymentTemplate and PaymentTemplateFacade, PaymentTemplateConfig, etc.
        $content = str_replace(self::TEMPLATE_PASCAL, $this->providerNamePascal, $content);

        // 2. camelCase - replaces paymentTemplate and paymentTemplateTransfer, paymentTemplateConfig, etc.
    ...
    private function showSuccess(): void
    {
        if ($this->projectMode) {
            ...
            $this->output("  3. Commit: git commit -m \"Rename from PaymentTemplate to {$this->providerNamePascal}\"\n");
...
// Execute
$renamer = new PaymentTemplateRenamer();
